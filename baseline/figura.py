...
V0, V1 = "#2a78d6", "#eb6834"          # slots categoricos 1 e 2, validados
...
assert len(todos0) == len(todos1), (len(todos0), len(todos1))
residuo = max(abs(a - b) for a, b in zip(todos0, todos1))
npontos = len(todos0)
...
phi1 = [p for p, _ in b1[bloco]][::3]
chi1 = [v for _, v in b1[bloco]][::3]
...
ax1.plot(phi1, chi1, ls="none", marker="o", ms=5.5, mfc=V1, mec="#fcfcfb",
         mew=1.2, label="V1 otimizado", zorder=3)
...
ax2.barh([i + alt/2 + 0.02 for i in y], t0, height=alt, color=V0,
         label="V0 base", zorder=2)
ax2.barh([i - alt/2 - 0.02 for i in y], t1, height=alt, color=V1,
         label="V1 otimizado", zorder=2)
for i, (a, b) in enumerate(zip(t0, t1)):
    ax2.text(a + 0.25, i + alt/2 + 0.02, f"{a:.2f}", va="center",
             fontsize=8.5, color=TINTA2)
    ax2.text(b + 0.25, i - alt/2 - 0.02, f"{b:.2f}", va="center",
             fontsize=8.5, color=TINTA2)
...
ax2.text(0.0, 1.005, "preparo serial 33,9 s $\\rightarrow$ 21,5 s · "
         "total 70 s $\\rightarrow$ 55,6 s",
