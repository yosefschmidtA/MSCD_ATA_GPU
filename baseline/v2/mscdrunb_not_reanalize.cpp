...
----------------------------------------------------------------------
*/
/* Hash exato para a deduplicacao do symtrivert.

   Exato, nao aproximado: as tres chaves ja chegam quantizadas
   (round(...,0.005f) e round(...,vlenc)), entao padrao de bits identico <=>
   valores iguais, que e' a comparacao que o codigo original faz com ==.
   Unica armadilha: -0.0f e +0.0f sao == mas tem bits diferentes, por isso a
   normalizacao abaixo -- sem ela o mesmo valor entraria duas vezes.

   O balde m entra na chave porque a escolha do balde usa akind (a especie do
   atomo do meio), que NAO faz parte da chave guardada: duas especies com a
   mesma geometria sao entradas distintas no original, e tem de continuar sendo. */
struct MscdTri { float l,v,c; int o; };

static bool mscd_menor(const MscdTri &a,const MscdTri &b)
{ if (a.c!=b.c) return a.c<b.c;
  if (a.v!=b.v) return a.v<b.v;
  return a.l<b.l;
}

static unsigned int mscd_mistura(unsigned int h)
{ h^=h>>16; h*=0x7feb352dU; h^=h>>15; h*=0x846ca68bU; h^=h>>16;
  return h;
}

static unsigned int mscd_chave(float a,float b,float c,int m)
{ union { float f; unsigned int u; } x;
  unsigned int h;
  if (a==0.0f) a=0.0f;
  if (b==0.0f) b=0.0f;
  if (c==0.0f) c=0.0f;
  x.f=a; h=mscd_mistura(x.u);
  x.f=b; h=mscd_mistura(h^(x.u*0x9e3779b9U));
  x.f=c; h=mscd_mistura(h^(x.u*0x85ebca6bU));
...
{ int ia,ib,ic,j,k,m,n,p,q,tscatter,analyze,slot,nslot;
  ...
  int *tempadd;
  float *tempar;
  /* origem[] segue de onde veio cada entrada enquanto a fase B embaralha os
     baldes; destino[] traduz slot-da-fase-A -> indice compactado final. Sem
     esses dois a fase C teria de redescobrir o indice varrendo natoms^3. */
  int *origem,*destino;
  int *htab,hcount;
  ...
  tempadd=NULL; tempar=NULL; origem=NULL; destino=NULL; nslot=0;
  htab=NULL; hsize=hmask=0; hcount=0; ordbuf=NULL;
  vlenc=kmin/100.0f;
  ...
  { ++analyze;
    ...
    { // o original refazia os new a cada Reanalyzing sem liberar: ~130 MB por volta
      if (tempadd) delete [] tempadd;
      if (tempar) delete [] tempar;
      if (origem) delete [] origem;
      if (destino) delete [] destino;
      nslot=nsymm*tscatter+1;
      tempadd=new int [nsymm]; tempar=new float [mscatter];
      origem=new int [nslot]; destino=new int [nslot];
      if (htab) delete [] htab;
      if (ordbuf) delete [] ordbuf;
      // estritamente maior que nslot: entradas <= nslot, logo nunca lota
      hsize=1024; while ((int)hsize<=nslot) hsize<<=1;
      hmask=hsize-1; hcount=0;
      htab=new int [hsize];
      ordbuf=new MscdTri [tscatter+2];
      if ((!tempadd)||(!tempar)||(!origem)||(!destino)||(!htab)||(!ordbuf))
        error=102;
      else
      { for (j=0;j<nsymm;++j) tempadd[j]=0;
        for (j=0;j<nslot;++j) destino[j]=-1;
  ...
  { n=m*tscatter;
    /* Eram tres selection sorts encadeados, O(n^2) cada, para chegar na ordem
       lexicografica por (cosbeta, 1/r2, r1). std::sort da o mesmo resultado:
       dentro do balde as tres chaves juntas sao unicas (a dedup garante), logo
       a ordem total e' a mesma e nao ha empate para desempatar. */
    q=tempadd[m];
    for (j=0;j<q;++j)
    { ordbuf[j].l=tempar[(n+j)*3]; ordbuf[j].v=tempar[(n+j)*3+1];
      ordbuf[j].c=tempar[(n+j)*3+2]; ordbuf[j].o=origem[n+j];
    }
    std::sort(ordbuf,ordbuf+q,mscd_menor);
    for (j=0;j<q;++j)
    { tempar[(n+j)*3]=ordbuf[j].l; tempar[(n+j)*3+1]=ordbuf[j].v;
      tempar[(n+j)*3+2]=ordbuf[j].c; origem[n+j]=ordbuf[j].o;
    }
  ...
  ntrieven=k;
  MSCDT("B ordena+compacta baldes");
  ...
  /* A fase A ja resolveu, para cada trio, QUAL assinatura ele e' -- guardou o
     slot em tevenadd. A fase B so' permutou os slots, e destino[] registrou a
     permutacao. Entao aqui basta traduzir. O original refazia a geometria dos
     natoms^3 trios e procurava o indice por varredura linear do balde: 13 s. */
  ...
  { if ((msorder==2)&&(patom[ia*12+7]==0)) continue;
    ...
    { if (ib==ia) continue;
      ...
      { if (ic==ib) continue;
        n=ia*natoms*natoms+ib*natoms+ic;
        slot=tevenadd[n];
        j=(slot>=0)?destino[slot]:-1;
        if (j<0) error=621;
        else
        { tevenadd[n]=j;
